...
def fmcw_pro(file_path, offset: float = 0.0):
    ...
    [no_diff, diff] = preprocess_corr(data)
    diff = np.expand_dims(diff, axis=0)
    no_diff = np.expand_dims(no_diff, axis=0)
    print(np.array(diff[:, :, :]).shape)
    print(np.array(no_diff[:, :, :]).shape)

    return np.array(no_diff[:, :, :]), np.array(diff[:, :, :])
...
if __name__ == "__main__":
    fmcw_pro("/Users/lily/Documents/myApps/Capstone_Saveme/15_aligner/tools/ultrasound/test_code/2.pcm")