...
Modality-specific feature extractors (each outputs 128-dim):
  ...
  EAR (600 samples)    : single 1D CNN      — 3 × Conv(3, stride 2)
...
Split strategy (22_models):
  Per participant, per class: shuffle → 60-20-20 (train-val-test), then global shuffle.
  No undersampling.
"""
...
from main import (
    MultiModalDataset, build_splits, print_confirm,
...
class PPGFeatureExtractor(nn.Module):
    ...
    Branch1 (kernel=3, dilation=1):
        Conv(3,32,s2) → Conv(3,64,s2) → Conv(3,128,s1)
    Branch2 (kernel=9, dilation {2,4,8}):
        Conv(9,32,s2,d2) → Conv(9,64,s2,d4) → Conv(9,128,s1,d8)
    Fusion:
        Concat → Conv(1,128) → GAP → 128-dim
    """
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        feat = torch.cat([b1[..., :L], b2[..., :L]], dim=1)
        feat = self.fusion(feat)
        return self.gap(feat).squeeze(-1)


class EARFeatureExtractor(nn.Module):
    """1D CNN for EAR signal. Input: (B, 600). Output: 128-dim."""
...
class UltrasoundFeatureExtractor(nn.Module):
    """2D CNN for ultrasound. Input: (B, 100, 1000). Output: 128-dim."""
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='Train multi-modal CNN (22_models). '
                    'Split: per-participant per-class shuffle → 60-20-20, no undersampling.'
    )
    # ── single-participant ────────────────────────────────────────────────────
    parser.add_argument('windows_dir', nargs='?', default='',
                        help='[Single] Windowed .npz directory with manifest.json.')
    parser.add_argument('--trials_json', default='',
                        help='[Single] Path to trials.json.')
    parser.add_argument('--participant_id', default='p0',
                        help='[Single] Participant ID label (default: p0).')
    # ── multi-participant ─────────────────────────────────────────────────────
    parser.add_argument('--participants_json', default='',
                        help='[Multi] JSON: [{"participant_id":..., "directory":...}, ...]')
    # ── shared ────────────────────────────────────────────────────────────────
    ...
    if args.participants_json:
        ...
        participants_cfg = [
            {
                ...
                'windows_dir':   os.path.join(p['directory'], 'windows'),
                'trials_json':   os.path.join(p['directory'], 'trials.json'),
            }
            for p in raw
        ]
    elif args.windows_dir and args.trials_json:
        participants_cfg = [
            {
        ...
        parser.error('Provide either --participants_json or (windows_dir + --trials_json).')

    # ── split ─────────────────────────────────────────────────────────────────
    train_ds, val_ds, test_ds, info = build_splits(
    ...
    def _eval_and_save(loader, ds_label: str, tag: str) -> dict:
        _, _, det = evaluate(model, loader, criterion, device, return_details=True)
        tl = [d[1] for d in det]
        pl = [d[2] for d in det]
        fi = compute_f1(tl, pl, num_classes)
        cm = sk_confusion_matrix(tl, pl, labels=list(range(num_classes)))
    ...
    test_f1 = _eval_and_save(test_loader, 'Test', 'test')
    logging.info(
        f"[Test] f1_macro={test_f1['f1_macro']:.4f} "
        f"f1_weighted={test_f1['f1_weighted']:.4f}"
    )
